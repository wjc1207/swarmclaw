...
#define MIMI_SECRET_SEARCH_KEY      ""              // 任意：Tavily Search APIキー
...
mimi> set_search_key tvly-...       # Tavily Search APIキーを設定
...
| ツール | 説明 |
|--------|------|
| `web_search` | Tavily Search APIでウェブ検索、最新情報を取得 |
...
ウェブ検索を有効にするには、`mimi_secrets.h`で[Tavily Search APIキー](https://tavily.com/)（`MIMI_SECRET_SEARCH_KEY`）を設定してください。
