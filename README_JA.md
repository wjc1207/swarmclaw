...
Telegramでメッセージを送ると、ESP32-S3がWiFi経由で受信し、エージェントループに送ります — LLMが思考し、ツールを呼び出し、メモリを読み取り — 返答を送り返します。**Anthropic (Claude)**、**OpenAI (GPT)**、**OpenRouter**、**NVIDIA NIM** の4種類をサポートし、実行時に切り替え可能です。すべてが$5のチップ上で動作し、データはすべてローカルのFlashに保存されます。
...
- **Anthropic APIキー** — [console.anthropic.com](https://console.anthropic.com)から取得、または **OpenAI APIキー** — [platform.openai.com](https://platform.openai.com)から取得、または **OpenRouter APIキー** — [openrouter.ai](https://openrouter.ai)から取得（無料枠あり）、または **NVIDIA NIM APIキー** — [integrate.api.nvidia.com](https://integrate.api.nvidia.com)から取得（無料枠あり）
...
#define MIMI_SECRET_MODEL_PROVIDER  "anthropic"     // "anthropic"、"openai"、"openrouter" または "nvidia"
#define MIMI_SECRET_SEARCH_KEY      ""              // 任意：検索APIキー
#define MIMI_SECRET_SEARCH_PROVIDER "tavily"        // "tavily"（デフォルト）または "brave"
...
mimi> set_model_provider openai    # プロバイダーを切替（anthropic|openai|openrouter|nvidia）
mimi> set_model gpt-4o             # LLMモデルを変更
mimi> set_proxy 127.0.0.1 7897    # HTTPプロキシを設定
mimi> clear_proxy                  # プロキシを削除
mimi> set_search_key tvly-...       # 検索APIキーを設定（TavilyまたはBrave）
mimi> set_search_provider brave    # 検索プロバイダーを切替（tavily|brave）
...
MimiClawはAnthropic、OpenAI、OpenRouter、NVIDIA NIM全てのツール呼び出しをサポート — LLMは会話中にツールを呼び出し、タスクが完了するまでループします（ReActパターン）。

| ツール | 説明 |
|--------|------|
| `web_search` | ウェブ検索で最新情報を取得（デフォルトTavily、オプションBrave） |
...
ウェブ検索を有効にするには、`mimi_secrets.h`で検索APIキー（`MIMI_SECRET_SEARCH_KEY`）を設定してください。デフォルトは[Tavily](https://tavily.com/)で、`MIMI_SECRET_SEARCH_PROVIDER`で[Brave Search](https://brave.com/search/api/)に切り替えることもできます。
...
- **マルチプロバイダー** — Anthropic (Claude)、OpenAI (GPT)、OpenRouter（無料枠あり）、NVIDIA NIM（無料枠あり）をサポート、実行時に切り替え可能
