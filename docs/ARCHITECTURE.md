...
4. Agent Loop (Core 1) pops message:
   ...
   d. ReAct loop (max 10 iterations):
      ...
      iii. If stop_reason == "tool_use":
           - Execute each tool (e.g. web_search → Tavily Search API)
...
│   └── tool_web_search.c   Tavily Search API via HTTPS (direct + proxy)
│
├── memory/
...
| `MIMI_SECRET_SEARCH_KEY`    | Tavily Search API key (optional)        |
...
| `agent/tools/*`             | `tools/tool_registry.c` + `tool_web_search.c` | web_search via Tavily API |
