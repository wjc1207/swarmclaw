                                  ...
                                  char *output, size_t output_size)
{
    ...
    char *mpy_output = NULL;
    esp_err_t err = mpy_runner_exec(path_buf, timeout_ms, &mpy_output);

    if (err == ESP_OK) {
        /* Escape output for JSON */
        cJSON *resp = cJSON_CreateObject();
        cJSON_AddBoolToObject(resp, "ok", 1);
        cJSON_AddStringToObject(resp, "output", mpy_output ? mpy_output : "");
        ...
        cJSON_AddStringToObject(resp, "error", mpy_output ? mpy_output : "unknown error");
        char *json_str = cJSON_PrintUnformatted(resp);
        if (json_str) {
            snprintf(output, output_size, "%s", json_str);
            free(json_str);
