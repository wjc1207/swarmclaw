...
你在 Telegram 发一条消息，ESP32-S3 通过 WiFi 收到后送进 Agent 循环 — LLM 思考、调用工具、读取记忆 — 再把回复发回来。同时支持 **Anthropic (Claude)**、**OpenAI (GPT)**、**OpenRouter** 和 **NVIDIA NIM** 四种提供商，运行时可切换。一切都跑在一颗 $5 的芯片上，所有数据存在本地 Flash。
...
- 一个 **Anthropic API Key** — 从 [console.anthropic.com](https://console.anthropic.com) 获取，或一个 **OpenAI API Key** — 从 [platform.openai.com](https://platform.openai.com) 获取，或一个 **OpenRouter API Key** — 从 [openrouter.ai](https://openrouter.ai)（提供免费额度），或一个 **NVIDIA NIM API Key** — 从 [integrate.api.nvidia.com](https://integrate.api.nvidia.com)（提供免费额度）
...
#define MIMI_SECRET_MODEL_PROVIDER  "anthropic"     // "anthropic"、"openai"、"openrouter" 或 "nvidia"
#define MIMI_SECRET_SEARCH_KEY      ""              // 可选：搜索 API key
#define MIMI_SECRET_SEARCH_PROVIDER "tavily"        // "tavily"（默认）或 "brave"
...
mimi> set_model_provider openai    # 切换提供商（anthropic|openai|openrouter|nvidia）
mimi> set_model gpt-4o             # 换模型
mimi> set_proxy 192.168.1.83 7897  # 设置代理
mimi> clear_proxy                  # 清除代理
mimi> set_search_key tvly-...       # 设置搜索 API Key（Tavily 或 Brave）
mimi> set_search_provider brave    # 切换搜索提供商（tavily|brave）
...
MimiClaw 同时支持 Anthropic、OpenAI、OpenRouter 和 NVIDIA NIM 的工具调用 — LLM 在对话中可以调用工具，循环执行直到任务完成（ReAct 模式）。

| 工具 | 说明 |
|------|------|
| `web_search` | 搜索网页，获取实时信息（默认 Tavily，可选 Brave） |
...
启用网页搜索需要在 `mimi_secrets.h` 中设置搜索 API key（`MIMI_SECRET_SEARCH_KEY`）。默认使用 [Tavily](https://tavily.com/)，也可通过 `MIMI_SECRET_SEARCH_PROVIDER` 切换为 [Brave Search](https://brave.com/search/api/)。
...
- **多提供商** — 同时支持 Anthropic (Claude)、OpenAI (GPT)、OpenRouter（免费额度可用）和 NVIDIA NIM（免费额度可用），运行时可切换
