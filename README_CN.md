...
#define MIMI_SECRET_SEARCH_KEY      ""              // 可选：Tavily Search API key
...
mimi> set_proxy 192.168.1.83 7897  # 设置代理
mimi> clear_proxy                  # 清除代理
mimi> set_search_key tvly-...       # 设置 Tavily Search API Key
...
| 工具 | 说明 |
|------|------|
| `web_search` | 通过 Tavily Search API 搜索网页，获取实时信息 |
...
启用网页搜索需要在 `mimi_secrets.h` 中设置 [Tavily Search API key](https://tavily.com/)（`MIMI_SECRET_SEARCH_KEY`）。
